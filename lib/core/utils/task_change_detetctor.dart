...
class TaskChangeDetector {
  bool hasChanges({
    ...
    return original.title != current.titleController.text.trim() ||
           ...
           !_listsEqual(original.subtaskIds, current.subtaskIds) ||  // Fixed: changed from current.subtaskId to current.subtaskIds
  ...
  /// Check if subtasks have been modified
  bool hasSubtaskChanges({
    required List<String> originalSubtaskIds,
    required List<String> currentSubtaskIds,
  }) {
    return !_listsEqual(originalSubtaskIds, currentSubtaskIds);
  }

  /// Check if only subtasks have changed (useful for partial updates)
  bool onlySubtasksChanged({
    required TaskModel original,
    required TaskEditFormData current,
  }) {
    final hasOtherChanges = original.title != current.titleController.text.trim() ||
           original.description != current.descriptionController.text.trim() ||
           original.projectId != current.selectedProjectId ||
           original.status != current.selectedStatus.index ||
           original.priority != current.selectedPriority.index ||
    ...
    final hasSubtaskChanges = !_listsEqual(original.subtaskIds, current.subtaskIds);
    
    return !hasOtherChanges && hasSubtaskChanges;
  }
}